...
def _generated_banner(source: str) -> str:
    """Return an HTML-comment banner marking a file as generated.

    Used as a fallback for files **without** YAML frontmatter.  For files
    with frontmatter, call :func:`_inject_generated_banner` instead, which
    embeds the banner as YAML comments **inside** the frontmatter so that
    the file still starts with ``---`` (required by Copilot / Claude
    plugin parsers and by this script's own ``_split_frontmatter``).
    """
    return (
        "<!--\n"
        f"AUTO-GENERATED by scripts/build_plugin.py from {source}.\n"
        "DO NOT EDIT this file directly — edit the source template and rerun the build.\n"
        "Regenerate locally:\n"
        "    python scripts/build_plugin.py --force\n"
        "Regenerate the CI-published dist (compact profile, sci- prefix):\n"
        "    python scripts/build_plugin.py -o dist/sciagent \\\n"
        "        --profile compact --format compact-marketplace --force --name-prefix sci\n"
        "-->\n"
    )


def _yaml_banner_lines(source: str) -> str:
    """Return banner text formatted as YAML ``#`` comment lines.

    The ruler uses ``=`` rather than ``-`` so the banner cannot collide
    with the ``---`` frontmatter delimiter (which ``_split_frontmatter``
    locates via a naive ``find("---", 3)``).
    """
    return (
        "# ======================================================================\n"
        f"# AUTO-GENERATED by scripts/build_plugin.py from {source}.\n"
        "# DO NOT EDIT this file directly — edit the source template and rerun.\n"
        "# Regenerate locally:\n"
        "#     python scripts/build_plugin.py --force\n"
        "# Regenerate the CI-published dist (compact profile, sci- prefix):\n"
        "#     python scripts/build_plugin.py -o dist/sciagent \\\n"
...
def _inject_generated_banner(content: str, source: str) -> str:
    """Insert a generation banner without disturbing YAML-frontmatter parsing.

    * If *content* begins with ``---`` (YAML frontmatter), the banner is
      injected as ``#`` comment lines **inside** the frontmatter, right
      after the opening ``---``.  This keeps line 1 as ``---`` so strict
      frontmatter parsers (Copilot, Claude Code, this script's own
      ``_split_frontmatter``) still recognise the file.
    * Otherwise, the HTML-comment banner is prepended.
    """
    ...
    if stripped.startswith("---"):
        ...
        if first_nl == -1:
            # Degenerate: file is just "---".  Fall back to HTML banner.
            return _generated_banner(source) + content
        opener = stripped[: first_nl + 1]  # "---\n"
        rest = stripped[first_nl + 1 :]
        return leading + opener + _yaml_banner_lines(source) + rest
    return _generated_banner(source) + content


def _write(path: Path, content: str, force: bool, source: str | None = None) -> None:
    ...
    if source is not None:
        content = _inject_generated_banner(content, source)
...
def _merge_agent_bodies(
    ...
    # Append domain expertise when provided (wizard-injected)
    if domain_expertise:
        merged_body = merged_body.rstrip() + "\n\n---\n\n" + domain_expertise + "\n"
...
def _merge_skill_bodies(
    ...
    Concatenates source skill bodies with optional section headers and
    synthesises a single top-level YAML frontmatter block (``name``,
    ``description``) so the merged file is a valid SKILL.md that
    plugin parsers can load.

    The merge spec's ``description`` is used verbatim; if it is None,
    the first source's description is inherited.  Source frontmatter
    blocks are stripped before concatenation so they do not appear
    embedded inside the merged body.
    """
    sources = spec["sources"]
    ...
    inherited_argument_hint: str | None = None
    for src_name in sources:
        ...
        # Strip per-source frontmatter so it does not end up embedded in
        # the merged body.  Inherit description / argument-hint from the
        # first source if the spec did not provide one.
        src_fm, src_body = _split_frontmatter(content)
        if src_fm and inherited_description is None:
            for line in src_fm.splitlines():
                if line.startswith("description:") and inherited_description is None:
                    inherited_description = line.split(":", 1)[1].strip()
                elif line.startswith("argument-hint:") and inherited_argument_hint is None:
                    inherited_argument_hint = line.split(":", 1)[1].strip()
        body = src_body.strip() if src_fm else content.strip()
    ...
    final_description = description if description else (inherited_description or "")
    fm_lines = [f"name: {merged_name}"]
    if final_description:
        # YAML scalar — escape any embedded double-quotes
        safe_desc = final_description.replace('"', '\\"')
        fm_lines.append(f'description: "{safe_desc}"')
    if inherited_argument_hint:
        safe_hint = inherited_argument_hint.replace('"', '\\"')
        fm_lines.append(f'argument-hint: "{safe_hint}"')
    fm_block = "---\n" + "\n".join(fm_lines) + "\n---\n\n"

    return fm_block + merged_body
...
def _build_agents(
    ...
    for merged_name, spec in sorted(merge_agents.items()):
        ...
        merge_src = "merged " + ", ".join(
            f"templates/agents/.github/agents/{s}.agent.md" for s in spec["sources"]
        )
        _write(dest, merged_content, force, source=merge_src)
        written.append(dest)
    ...
    for src_file in sorted(AGENTS_SRC.glob("*.agent.md")):
        ...
        _write(
            ...
            output_content,
            force,
            source=src_file.relative_to(REPO_ROOT).as_posix(),
        )
        written.append(dest)
...
def _build_skills(
    ...
    for merged_name, spec in sorted(merge_skills.items()):
        ...
        merge_src = "merged " + ", ".join(
            f"templates/skills/{s}/SKILL.md" for s in spec["sources"]
        )
        _write(dest, merged_content, force, source=merge_src)
        written.append(dest)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to a Markdown file whose contents are appended to every "
            "agent body as domain expertise.  Typically generated by the wizard."
        ),
    )
    parser.add_argument(
        "--extra-skills-dir",
        default=None,
        help=(
            "Directory containing additional skill folders (<name>/SKILL.md) "
            "to copy into the plugin's skills/ directory.  These are added "
            "alongside — not filtered by — the active profile."
        ),
    )
    parser.add_argument(
        "--extra-docs-dir",
        default=None,
        help=(
            "Directory containing additional .md documentation files to copy "
            "into the plugin's docs/ directory (e.g. package documentation)."
        ),
    )
...
# ---------------------------------------------------------------------------
# Extra skills / docs helpers (for wizard subprocess integration)
# ---------------------------------------------------------------------------

def _collect_extra_skill_names(extra_skills_dir: Path | None) -> list[str]:
    """Return slug names for all SKILL.md files found in extra_skills_dir."""
    if not extra_skills_dir or not extra_skills_dir.is_dir():
        return []
    names: list[str] = []
    for child in sorted(extra_skills_dir.iterdir()):
        if child.is_dir() and (child / "SKILL.md").is_file():
            names.append(child.name)
    return names


def _copy_extra_skills(
    extra_skills_dir: Path | None,
    output: Path,
    force: bool,
) -> list[Path]:
    """Copy wizard-generated skill directories into the build output's skills/ folder."""
    if not extra_skills_dir or not extra_skills_dir.is_dir():
        return []
    target_base = output / "skills"
    ...
    for child in sorted(extra_skills_dir.iterdir()):
        ...
        if dest.exists():
            if force:
                ...
                raise FileExistsError(f"Extra skill dir already exists: {dest}")
...
def _copy_extra_docs(
    extra_docs_dir: Path | None,
    output: Path,
    force: bool,
) -> list[Path]:
    """Copy wizard-generated documentation files into the build output's docs/ folder."""
    if not extra_docs_dir or not extra_docs_dir.is_dir():
        return []
    target_base = output / "docs"
    target_base.mkdir(parents=True, exist_ok=True)
    copied: list[Path] = []
    for item in sorted(extra_docs_dir.iterdir()):
        dest = target_base / item.name
        if item.is_file():
            if dest.exists() and not force:
                raise FileExistsError(f"Extra doc already exists: {dest}")
            ...
            if dest.exists():
                if force:
                    ...
                    raise FileExistsError(f"Extra docs dir already exists: {dest}")
...
def _build_copilot(
    ...
    # Collect extra skill names for plugin.json (before building)
    extra_skill_names = _collect_extra_skill_names(extra_skills_dir)
    all_skill_names = skill_names + extra_skill_names

    plugin_json = _build_plugin_json(
        output, version, project_meta, agent_names, all_skill_names,
    ...
    agent_files = _build_agents(
        ...
        fullstack=args.fullstack, domain_expertise=domain_expertise,
    )
    skill_files = _build_skills(
        output, replacements, args.force, profile=profile,
    )

    # Copy extra skills (wizard-generated, not subject to profile filtering)
    extra_skill_files = _copy_extra_skills(extra_skills_dir, output, args.force)
    skill_files.extend(extra_skill_files)
    ...
    _rewrite_routing_table(agent_files, profile, name_prefix, all_skill_names)
    ...
    # Copy extra docs (wizard-generated package documentation)
    extra_doc_files = _copy_extra_docs(extra_docs_dir, output, args.force)

    readme = _build_readme(output, version, agent_names, all_skill_names, name_prefix, args.force)
    ...
    if extra_doc_files:
        ...
        for f in extra_doc_files:
            print(f"  {f.relative_to(output)}")
    total = 2 + len(agent_files) + len(skill_files) + len(template_files) + len(extra_doc_files)
    print(f"  Total: {total} files")
    print(
...
def _build_claude(
    ...
    # Collect extra skill names for plugin.json
    extra_skill_names = _collect_extra_skill_names(extra_skills_dir)
    all_skill_names = skill_names + extra_skill_names
    ...
    # Copy extra skills (wizard-generated)
    extra_skill_files = _copy_extra_skills(extra_skills_dir, output, args.force)
    skill_files.extend(extra_skill_files)
    ...
    readme = _build_claude_readme(
        output, version, agent_names, all_skill_names, name_prefix, args.force,
    )
...
def main() -> None:
    ...
    # Load optional domain expertise text
    domain_expertise = ""
    if args.domain_expertise_file:
        dep = Path(args.domain_expertise_file)
        if not dep.is_file():
            print(f"ERROR: domain expertise file not found: {dep}", file=sys.stderr)
            sys.exit(1)
        domain_expertise = dep.read_text(encoding="utf-8").strip()

    # Resolve optional extra directories
    extra_skills_dir: Path | None = None
    if args.extra_skills_dir:
        extra_skills_dir = Path(args.extra_skills_dir).resolve()
        if not extra_skills_dir.is_dir():
            print(f"ERROR: extra skills dir not found: {extra_skills_dir}", file=sys.stderr)
            sys.exit(1)

    extra_docs_dir: Path | None = None
    if args.extra_docs_dir:
        extra_docs_dir = Path(args.extra_docs_dir).resolve()
        if not extra_docs_dir.is_dir():
            print(f"ERROR: extra docs dir not found: {extra_docs_dir}", file=sys.stderr)
            sys.exit(1)
